package com.brave.mydragdropcomposeapp.theme
...
class PagerLazyFling(
    ...
    override suspend fun ScrollScope.performFling(initialVelocity: Float): Float {
        ...
        return if (abs(initialVelocity) >= 0f) {
            ...
            destValue = if (abs(velocityX) < 500) {
                if (childLeft < childWidth / 2) {
                    scrollValue.toFloat() - childLeft
                } else {
                    scrollValue.toFloat() - childLeft + childWidth;
                }
            } else {
                if (velocityX < 0)
                    scrollValue.toFloat() - childLeft
            ...
            animate(
                scrollValue.toFloat(), destValue, 0f, tween(
                    300,
